...
const isAdmin = (usuario) => {
  ...
  return ADMIN_EMAILS.includes(userEmail) || usuario.admin === true;
};
// ============================
  ...
  try {
    ...
    for (const card of cards) {
      try {
        ...
        console.error(
          `❌ Erro ao atualizar card ${card.id}:`,
          updateError.message,
        );
      }
    }
    ...
    const cardsAtualizados = snapshotAtualizado.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
...
export const criarCard = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.log(
        `⚠️ Usuário com email ${email} não encontrado no Firebase Auth`,
      );
    }
...
export const atualizarCard = async (req, res, next) => {
  try {
    ...
    if (!card.exists) {
      return res
        .status(404)
        .json({ success: false, error: "Card não encontrado" });
    }
  ...
  try {
    ...
    if (!isAdmin(usuario)) {
      return res
        ...
        .json({
          success: false,
          error: "Apenas administradores podem excluir cards",
        });
    }
...
export const adicionarTarefa = async (req, res, next) => {
  try {
    ...
    console.log(
      `🔍 Comparação: "${cardData.email}" === "${usuario.email}" = ${cardData.email === usuario.email}`,
    );
    ...
    // Validação dos dados
    const validation = validarTarefa({
      titulo,
      descricao,
      dataInicio,
      dataFim,
    ...
    console.log(
      `📅 Data hoje: ${hoje}, Data fim: ${dataFim}, Status: ${status}`,
    );

    const { recorrencia = RECORRENCIA.NENHUMA } = req.body;
    ...
    console.log(
      `✅ Tarefa adicionada com sucesso! Total de tarefas: ${tarefas.length}`,
    );

    // Verificar se a tarefa já está atrasada para notificar
    if (status === "atrasada") {
      console.log(
        `📧 Tarefa atrasada, enviando notificação para ${cardData.email}`,
      );
      try {
        const { enviarNotificacaoAtraso } =
          await import("../services/emailService.js");
        await enviarNotificacaoAtraso(
          cardData.email,
          novaTarefa,
          cardData.responsavel,
        );
  ...
  try {
    ...
    if (!admin) {
      return res
        ...
        .json({
          success: false,
          error: "Apenas administradores podem editar tarefas",
        });
    }
    ...
    tarefaAtualizada.status =
      tarefaAtualizada.dataFim < hoje && tarefaAtualizada.status !== "concluida"
        ? "atrasada"
        : tarefaAtualizada.status;
    ...
    if (
      tarefaAtualizada.status === "atrasada" &&
      !tarefaAtualizada.notificadoAtraso
    ) {
      const { enviarNotificacaoAtraso } =
        await import("../services/emailService.js");
      await enviarNotificacaoAtraso(
...
export const toggleConcluirTarefa = async (req, res, next) => {
  try {
    ...
    const novoStatus =
      tarefas[index].status === "concluida" ? "pendente" : "concluida";
